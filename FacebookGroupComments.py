...
def request_until_succeed(url):
    ...
    while success is False:
        try:
            ...
            print(e)
            time.sleep(attempt*60 + 30)

            print("Error for URL {}: {}".format(url, datetime.datetime.now()))
            print("Retrying: Attempt #{}, delaying for {} seconds.".format(attempt, attempt*60+30))
            attempt += 1
...
def getFacebookCommentFeedUrl(base_url):
    ...
    fields = r"&fields=id,message,reactions.limit(0).summary(true)" \
             r",created_time,comments,from,attachment"
    url = base_url + fields
...
def scrapeFacebookPageFeedComments(page_id, access_token, date):
    with open('{}_facebook_comments_{}.csv'.format(file_id, date), 'w', encoding="utf8") as file:
        w = csv.writer(file)
        ...
        with open('{}_facebook_statuses_{}.csv'.format(file_id, date), 'r', encoding="utf8") as csvfile:
            ...
            for status in reader:
                ...
                while has_next_page:
                    ...
                    for comment in comments['data']:
                            ...
                            while has_next_subpage:
                                ...
                                for sub_comment in sub_comments['data']:
                                    ...
                                    if num_processed % 100 == 0:
                                        print("{} Comments Processed for {}: {}".format(
                                            num_processed,
                        ...
                        if num_processed % 100 == 0:
                            print("{} Comments Processed for {}: {}".format(
                                num_processed, date, datetime.datetime.now()))
...
def get_comments_from_date(auth, input_date):
    print(f'Scraping comments from from {input_date}')
    scrapeFacebookPageFeedComments(auth['group_id'], auth['access_token'], input_date)
...
if __name__ == '__main__':
    ...
    d_start = datetime.date(2016, 9, 11)
    total = 1000

    dates = []
    for delta in range(total):
        current_date = d_start - datetime.timedelta(days=delta)
        current_date_str = current_date.__str__().replace('-', '_')
        dates.append(current_date_str)

    for input_date in dates:
        get_comments_from_date(auth, input_date)
