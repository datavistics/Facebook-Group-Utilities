...
def scrapeFacebookPageFeedStatus(group_id, access_token, since_date, until_date):
    until_date_str = str(until_date).replace('-', '_')
    with open('{}_facebook_statuses_{}.csv'.format(group_id, until_date_str), 'w', encoding='utf-8') as file:
        w = csv.writer(file)
...
if __name__ == '__main__':
    ...
    with open(r'C:\Users\Derek-T\PycharmProjects\group_posts\authentication.json') as file:
        auth = json.load(file)

    # import os
    # os.chdir('Statuses')

    d_end = datetime.date(2016, 9, 8)
    d_start = datetime.date(2015, 1, 2)

    for delta in range((d_end - d_start).days):
        d_0 = d_end + datetime.timedelta(days=delta - 1)
        d_1 = d_end + datetime.timedelta(days=delta)

        d_0_str = d_0.__str__()
        d_1_str = d_1.__str__()
        print(f'Scraping from {d_0_str} to {d_1_str}')
        scrapeFacebookPageFeedStatus(auth['group_id'], auth['access_token'], d_0_str, d_1_str)
        FacebookGroupComments.get_comments_from_date(auth, d_1_str)
